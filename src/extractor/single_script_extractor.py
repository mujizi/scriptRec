...
# 确保必要的目录存在
os.makedirs("/opt/Filmdataset/demo/script", exist_ok=True)
os.makedirs("/opt/Filmdataset/demo/clean", exist_ok=True)
...
- 根据所给剧本，分别总结剧本以下信息：
   ...
   - 剧本摘要(script_summary)：
        - 将script_theme、script_genre、script_type、script_subtypes、script_background、script_synopsis、script_structure七者结合，形成一个完整的剧本摘要。以一句话描述，字数50-100字。
   - 输出样例
    {
        "script_name":"XXX",
        "script_theme":"XXX",
...
def save_to_excel(script_data: Dict[str, Any], output_path: str = "/opt/Filmdataset/demo/script/script.xlsx") -> None:
    """将剧本信息保存到Excel（明确指定openpyxl引擎）"""
    try:
        # 确保输出目录存在
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        
        data = [script_data] if isinstance(script_data, dict) else script_data
        columns = [
...
def create_interface():
    ...
    def process_script(script_name):
        ...
        script_path = os.path.join("/opt/Filmdataset/demo/clean", script_name)
        
        if not os.path.exists(script_path):
            return f"错误: 文件 '{script_name}' 不存在", []
        
        try:
            script_data = asyncio.run(extract_script_information(PROMPT, script_path))
            print_script_info(script_data)
            save_to_excel(script_data)
    ...
    def refresh_file_list():
        ...
        if not script_files:
            return gr.Dropdown.update(choices=[], value=None), gr.Markdown("请上传txt剧本到/opt/Filmdataset/demo/clean")
    ...
    with gr.Blocks(title="剧本信息提取系统") as interface:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                script_dropdown = gr.Dropdown(
                    ...
                    label="选择剧本文件",
                    value=script_files[0] if script_files else None
                )
                refresh_btn = gr.Button("🔄 刷新文件列表", variant="secondary")
                process_btn = gr.Button("提取剧本信息", variant="primary")
        ...
        refresh_btn.click(
            ...
            inputs=[],
            outputs=[script_dropdown, status_output]
        )
        
        if not script_files:
            gr.Markdown("提示：请将txt格式剧本上传到 `/opt/Filmdataset/demo/clean` 目录")
