...
def create_milvus_collection(client, collection_name, summary_embeddings):
    ...
    scene_name_field = FieldSchema(name="scene_name", dtype=DataType.VARCHAR, max_length=5000)
    scene_specifics_field = FieldSchema(name="scene_specifics", dtype=DataType.VARCHAR, max_length=60000)
    scene_summary_field = FieldSchema(name="scene_summary", dtype=DataType.VARCHAR, max_length=5000, enable_analyzer=True)
    script_name_field = FieldSchema(name="script_name", dtype=DataType.VARCHAR, max_length=5000)

    # Create the collection schema with all fields
    schema = CollectionSchema(fields=[id_field, dense_vector_field, scene_name_field, scene_specifics_field, scene_summary_field, script_name_field],
...
def insert_dense_data_to_collection(client, collection_name, docs_embeddings, content_list, batch_size=100):
    ...
    for i in range(len(content_list)):
        ...
        entity = {
            ...
            "dense_vector": row,
            "scene_name": content_list[i][0],  # 注意索引调整，因为不再包含Excel的id列
            "scene_specifics": content_list[i][1],
            "scene_summary": content_list[i][2],
            "script_name": content_list[i][3],
            # sparse_vector由系统生成
        }
        # print('entity,', entity)
...
def vector_query(client, collection_name, text, top_k=1):
    ...
    try:
        ...
        search_res = client.search(
            ...
            output_fields=["scene_name", "scene_specifics", "scene_summary", "script_name"],
        )

        if search_res:
            search_res = search_res[0][0]["entity"]
            print(f"Search successful. Result: {search_res.get('scene_name', 'No result')}")
        else:
...
if __name__ == "__main__":
    ...
    try:
        # 遍历指定目录下的所有 xlsx 文件
        directory = '/opt/Filmdataset/demo/output'
        ...
        for filename in all_files:
            if filename not in processed_files:
                ...
                try:
                    ...
                    # 从原始数据中提取其他字段（不包含id列）
                    data_list = df[['场景名', '场景具体内容', '摘要', '剧本名']].values.tolist()
                    # print(data_list)
    ...
    if res:
        print("\n查询结果:")
        print(f"场景名: {res['scene_name']}")
        print(f"场景摘要: {res['scene_summary']}")
        print(f"剧本名: {res['script_name']}")
    else:
        print("未找到匹配结果。")
